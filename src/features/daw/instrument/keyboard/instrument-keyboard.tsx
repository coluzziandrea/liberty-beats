...
import { selectPlayingTrackKeys } from '../store/selectors'
...
export const InstrumentKeyboard = (props: InstrumentKeyboardProps) => {
  ...
  const playingTrackKeys = useSelector(selectPlayingTrackKeys)

  const currentTrackPlayingKeys =
    playingTrackKeys.find((item) => item.trackId === props.selectedTrack.id)
      ?.keys || []
